...
export class PlatformPercentages extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save PlatformPercentages entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type PlatformPercentages must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("PlatformPercentages", id.toString(), this);
    }
  }

  static load(id: string): PlatformPercentages | null {
    return changetype<PlatformPercentages | null>(
      store.get("PlatformPercentages", id)
    );
  }
  ...
  get PenaltyFee(): BigInt {
    let value = this.get("PenaltyFee");
    return value!.toBigInt();
  }

  set PenaltyFee(value: BigInt) {
    this.set("PenaltyFee", Value.fromBigInt(value));
  }
